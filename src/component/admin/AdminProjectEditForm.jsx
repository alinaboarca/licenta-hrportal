...
export const AdminProjectEditForm = (props) => {
  ...
  const handleProjectSubmit = (event) => {
    ...
    axios
      .put(
        `http://localhost:3002/projects/${props.editData.ProjectId}`,
        formData
      )
      ...
      .then((res) => {
        ...
        originalEmps.forEach(async (emp) => {
          if(selectedEmps.indexOf(emp) === -1) {
            console.log('deleteing', emp);
            console.log('deleteing', props.editData.ProjectId);

            await axios.delete(`http://localhost:3002/deleteprojects/emp/${props.editData.ProjectId}/${emp}`);
          }
    ...
    axios
      ...
      .then((response) => {
        ...
        let value = Array.from(
          response.data,
          (option) => option.Employee.EmployeeId
        );
        console.log("selected", value);
        setSelectedEmps(value);
        setOriginalEmps(value);
      })
      .catch((error) => {
        console.log(error);
      });
